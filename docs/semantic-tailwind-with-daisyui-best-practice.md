...
```html
<button
  class="btn data-[state=open]:ring data-[state=open]:bg-primary data-[state=open]:text-primary-content"
>
  Toggle
</button>
```
...
> **Note:** Variants like `data-[state=open]:…` work with **Tailwind utilities**
> (e.g., `bg-primary`, `ring-2`). They won’t magically prefix non‑utility
> classes such as `btn-primary`. If you need to flip a daisyUI variant by state,
> compute the class in your component (`clsx(isOn && "btn-primary")`).

---

## 6) `@apply` vs `@utility` (v4 reality)

- Use **`@apply`** to inline Tailwind **utilities** into CSS when you must style
  third‑party DOM, author CSS Modules / Vue `<style>` blocks, or reduce
...
> `bg-base-200/60` or `text-base-content/70`) alongside a semantic class, place
> the stateful selectors in the `@layer utilities` block so they compile _after_
> the inline utilities. Otherwise those utilities will win the cascade and your
> state styles will never show up.
...
with tokenised utilities for consistent theming and good contrast.
...
- If a `bg-foo-500`‑style class doesn’t exist, confirm you declared the token in
