...
`POST /api/v1/routes` validates the request, checks Redis for a cache hit, and
enqueues a `GenerateRouteJob` when work is required. Responses follow the
sequence below:
...
use crate::domain::route::{RoutePlan, RouteRequest};

#[async_trait]
pub trait RouteRepository: Send + Sync {
    async fn save(&self, plan: &RoutePlan) -> anyhow::Result<()>;
    async fn find(&self, request_id: &str) -> anyhow::Result<Option<RoutePlan>>;
}

#[async_trait]
pub trait RouteService: Send + Sync {
    async fn request_route(&self, request: RouteRequest) -> anyhow::Result<RoutePlan>;
}

pub struct RouteServiceImpl<R: RouteRepository> {
    repo: Arc<R>,
}

impl<R: RouteRepository> RouteServiceImpl<R> {
    pub fn new(repo: Arc<R>) -> Self { Self { repo } }
}

#[async_trait]
impl<R: RouteRepository> RouteService for RouteServiceImpl<R> {
    async fn request_route(&self, request: RouteRequest) -> anyhow::Result<RoutePlan> {
...
pub async fn create_route(
    ...
    let plan = service.request_route(request).await?;
    Ok(HttpResponse::Accepted().json(plan))
}
```

Adapters remain thin translators and the domain owns all behaviour. Follow the
same pattern for user management, job dispatch, and caching ports to complete
the migration.
