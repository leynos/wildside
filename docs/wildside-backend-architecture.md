...
- **External APIs:** For certain features, external services like the
  **Overpass API** are consulted (e.g. to enrich point-of-interest
  data)([3](https://github.com/leynos/wildside/blob/9aa9fcecfdec116e4b35b2fde63f11fa7f495aaa/docs/wildside-backend-design.md#L46-L49)
  )(
  [3](https://github.com/leynos/wildside/blob/9aa9fcecfdec116e4b35b2fde63f11fa7f495aaa/docs/wildside-backend-design.md#L632-L640)).
...
When adding a new adapter (inbound or outbound), use this checklist to keep the
boundary explicit:
...
Module boundaries are enforced by a repo-local lint that runs during
`make lint`:
...
- **2025-12-15:** Introduce driving-port traits (`LoginService`, `UsersQuery`,
  ...
  integration guardrail tests (`backend/tests/adapter_guardrails.rs`) to
  exercise HTTP and WebSocket paths against deterministic test doubles.
...
| `POST`   | `/api/v1/routes/{route_id}/notes`       | Upsert a route note (idempotent).                                                                                                                                                                            | Session cookie       |
| `PUT`    | `/api/v1/routes/{route_id}/progress`    | Update route progress (idempotent).                                                                                                                                                                          | Session cookie       |
...
transport-agnostic. The inbound HTTP adapter applies the Actix `ResponseError`
mapping in `backend/src/inbound/http/error.rs`, translating error codes to HTTP
status codes, propagating the `trace-id` header when present, and redacting
`InternalError` payloads to the generic message "Internal server error".

### PWA data model alignment

The Progressive Web App (PWA) data model described in
`docs/wildside-pwa-data-model.md` introduces catalogue read models, descriptor
registries, and local-first user state. The backend must expose these shapes
through ports and repositories so the frontend can switch from fixtures to API
data without rewriting view components.

#### Domain modules

- `domain::catalogue` for route summaries, categories, themes, collections,
  trending highlights, and community picks.
- `domain::descriptors` for interest themes, tags, badges, safety toggles,
  and safety presets.
- `domain::routing` for route requests, route plans, stops, and POI
  projections.
- `domain::user` for profiles and preferences (interests, safety toggles,
  and unit system).
- `domain::annotations` for route notes and progress.
- `domain::offline` for offline bundle manifests (metadata only).
- `domain::walks` for walk sessions and completion summaries.

#### Driven ports (repositories)

- `CatalogueRepository` (read): returns `ExploreCatalogueSnapshot` plus
  catalogue entities used by Explore and Discover.
- `DescriptorRepository` (read): returns descriptor registries for interest
  themes, tags, badges, safety toggles, and safety presets.
...
- `ExampleDataRunsRepository` (read/write): records which example data seeds
  ...
  existence checks.
- `ExampleDataSeedRepository` (write): applies example data within a single
  transaction, recording the seed run and upserting users plus preferences.
...
- `ExploreCatalogueQuery` composes catalogue and descriptor repositories into
  a single Explore snapshot.
- `UserPreferencesCommand` replaces interest-only updates with preference
  updates that include safety toggles and units.
- `RouteAnnotationsCommand` handles note upserts and progress updates with
  idempotency checks.
...
signing key for operational visibility. The fingerprint is the first 8 bytes of
the hash encoded as 16 hexadecimal characters. Example log entry:
...
`User::try_from_strings`) that validate invariants such as UUID formats or
non-empty display names, ensuring adapters cannot introduce invalid state. The
domain may add lighter convenience wrappers (for example `User::from_strings`)
when the inputs are compile-time constants, but runtime code is expected to
call the fallible constructors and propagate the domain errors. These
invariants mirror the validation rules captured in the AsyncAPI spec we
maintain for events (e.g., the display name regex and length documented in the
spec)(
[6](https://github.com/leynos/wildside/blob/9aa9fcecfdec116e4b35b2fde63f11fa7f495aaa/spec/asyncapi.yaml#L190-L198)).
...
#### PWA catalogue and user state extensions

The PWA data model introduces catalogue read models, descriptor registries, and
local-first user state. The planned persistence additions include:
...
`401`, `limit` and `before` decoding or validation failures return `400`, and
provenance repository availability failures return `503` so operators can
distinguish transport availability from client input errors.
...
the 3.4.3 enrichment provenance endpoint is the `before`/`nextBefore` model
described above; no `cursor` query parameter or `nextCursor` response field is
implemented yet.

When Phase 4 work starts, the migration must preserve compatibility for
existing clients that currently use `before` and `nextBefore`.

Future compatibility requirements:

- Keep deterministic ordering on `(imported_at DESC, id DESC)` in both
  repository queries and supporting SQL indexes. Any future cursor filters must
  use the same composite key so page traversal remains lossless.
- Introduce `cursor` only as an additive transition on top of the existing
  `before` contract.
- Treat `before` and `cursor` as mutually exclusive once dual-mode support
  lands. Requests that provide both must return `400` with a validation error
  payload.
- Preserve endpoint-specific limit guardrails (`default 50`, `max 200`) even
  when the shared pagination crate defaults differ.
- Add any future cursor-mode response data without removing legacy fields
  during the transition. Existing clients must continue to parse
  `{ records, nextBefore? }` until deprecation is complete.
- Map future pagination decoding and direction errors to HTTP `400` and keep
...
3. Add transitional response fields such as `nextCursor` while retaining
   `nextBefore`.
...
- **POI and static data caching:** Although the database is quite capable, we
  ...
  POIs)”(
  [3](https://github.com/leynos/wildside/blob/9aa9fcecfdec116e4b35b2fde63f11fa7f495aaa/docs/wildside-backend-design.md#L681-L688)).
