...
| `GET`    | `/api/v1/admin/enrichment/provenance`   | List persisted enrichment provenance rows with `limit` and `before` query params. Returns `400` for validation failures and `503` when the reporting repository is unavailable. | Admin session cookie |
