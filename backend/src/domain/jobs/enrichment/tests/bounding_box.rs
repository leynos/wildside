...
proptest! {
    ...
    fn inverted_bounding_box_longitude_ordering_is_rejected(
        min_lng in -179.0_f64..180.0,
        min_lat in -89.0_f64..89.0,
        lat_span in 0.001_f64..1.0,
        inverted_lng_span in 0.001_f64..1.0,
    ) {
        let max_lng = (min_lng - inverted_lng_span).max(-180.0);
        let error = BoundingBox::new(min_lng, min_lat, max_lng, min_lat + lat_span)
            .expect_err("inverted longitudes should be rejected");
    ...
    fn equal_bounding_box_longitudes_are_rejected(
        ...
        let error = BoundingBox::new(lng, min_lat, lng, min_lat + lat_span)
