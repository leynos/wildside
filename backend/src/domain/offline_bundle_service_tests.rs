...
) -> OfflineBundleService<MockOfflineBundleRepository, FixtureIdempotencyRepository> {
    OfflineBundleService::new(Arc::new(repo), Arc::new(FixtureIdempotencyRepository))
}
...
async fn upsert_with_idempotency_stores_bundle_mutation_record() {
    ...
    let payload_hash = OfflineBundleService::<MockOfflineBundleRepository, MockIdempotencyRepository>::
    ...
    let service = OfflineBundleService::new(Arc::new(repo), Arc::new(idempotency_repo));
    let response = service
        .upsert_bundle(UpsertOfflineBundleRequest {
            user_id,
            bundle: payload,
