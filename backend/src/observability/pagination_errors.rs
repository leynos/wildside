//! Pagination cursor error observability.
//!
//! Adapters call [`record_pagination_error`] when they map a cursor failure
//! to the shared domain error envelope. The function emits a structured
//! `tracing` log and, when the `metrics` feature is enabled, increments the
//! `wildside_pagination_errors_total` Prometheus counter. The counter is
//! registered through [`register_pagination_error_metrics`] during process
//! start-up.
