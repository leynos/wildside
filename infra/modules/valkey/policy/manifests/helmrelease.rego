...
is_valkey(doc) if {
	object.get(labels(doc), "app.kubernetes.io/part-of", "") == "valkey"
}
...
deny contains msg if {
	...
	msg := sprintf("Valkey HelmRelease %s must pin chart.spec.version", [name])
}
...
deny contains msg if {
	...
	name := object.get(metadata(doc), "name", "<unknown>")
	msg := sprintf("Valkey HelmRelease %s must set chart.spec.sourceRef.name", [name])
}
...
deny contains msg if {
	...
	name := object.get(metadata(doc), "name", "<unknown>")
	msg := sprintf("Valkey HelmRelease %s must set chart.spec.sourceRef.namespace", [name])
}
