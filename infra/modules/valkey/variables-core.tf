...
variable "namespace" {
  description = "Namespace where the Valkey cluster will run"
  type        = string
  default     = "valkey"
...
variable "operator_namespace" {
  description = "Namespace where the Valkey operator will be installed"
  type        = string
  default     = "valkey-system"
...
variable "helm_release_name" {
  description = "Name assigned to the Valkey operator Helm release"
  type        = string
...
variable "chart_repository" {
  description = "Helm repository hosting the valkey-operator chart"
  type        = string
  default     = "oci://ghcr.io/hyperspike/helm"
...
variable "chart_name" {
  description = "Name of the Helm chart used to install the Valkey operator"
  type        = string
  default     = "valkey-operator"
...
variable "chart_version" {
  description = "Exact Helm chart version for the Valkey operator"
  type        = string
  default     = "0.0.60"
  ...
  validation {
    ...
    error_message = "chart_version must be a semantic version (e.g., 0.0.60)"
  }
}
...
variable "helm_values" {
  description = "Inline YAML values passed to the Valkey operator Helm release"
  type        = list(string)
  default     = []
  nullable    = false
...
variable "flux_helm_repository_name" {
  description = "Flux HelmRepository name for the valkey-operator chart"
  type        = string
  default     = "valkey-operator"
