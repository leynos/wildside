...
is_main_release(doc) if {
	...
	object.get(values_map, "cainjector", null) != null
}

is_webhook_release(doc) if {
	chart := lower(chart_name(doc))
	contains(chart, "webhook")
}

is_webhook_release(doc) if {
	group_name := object.get(values(doc), "groupName", "")
	group_name != ""
}

webhook_replica_count(doc) := object.get(object.get(values(doc), "webhook", {}), "replicaCount", 0)

cainjector_replica_count(doc) := object.get(object.get(values(doc), "cainjector", {}), "replicaCount", 0)
...
deny contains msg if {
	...
	msg := sprintf("Cert-manager HelmRelease %s must pin chart.spec.version", [name])
}
...
deny contains msg if {
	...
	name := object.get(metadata(doc), "name", "<unknown>")
	msg := sprintf("Cert-manager HelmRelease %s must set chart.spec.sourceRef.name", [name])
}
...
deny contains msg if {
	...
	name := object.get(metadata(doc), "name", "<unknown>")
	msg := sprintf("Cert-manager HelmRelease %s must set chart.spec.sourceRef.namespace", [name])
}
...
deny contains msg if {
	...
	name := object.get(metadata(doc), "name", "<unknown>")
	msg := sprintf("Cert-manager HelmRelease %s must set install.crds", [name])
}

deny contains msg if {
	doc := helmreleases[_]
	is_main_release(doc)
	replicas := object.get(values(doc), "replicaCount", null)
	replicas == null
	name := object.get(metadata(doc), "name", "<unknown>")
	msg := sprintf("Cert-manager HelmRelease %s must set values.replicaCount", [name])
}
...
deny contains msg if {
	...
	name := object.get(metadata(doc), "name", "<unknown>")
	msg := sprintf("Cert-manager HelmRelease %s must set values.webhook.replicaCount", [name])
}

deny contains msg if {
	doc := helmreleases[_]
	is_main_release(doc)
	cainjector_replica_count(doc) <= 0
	name := object.get(metadata(doc), "name", "<unknown>")
	msg := sprintf("Cert-manager HelmRelease %s must set values.cainjector.replicaCount", [name])
}
...
deny contains msg if {
	doc := helmreleases[_]
	is_main_release(doc)
	cainjector_replica_count(doc) > 1
	instance := object.get(metadata(doc), "name", "")
	not has_pdb("cainjector", instance)
	msg := "Cert-manager cainjector replicas > 1 require a PodDisruptionBudget"
}

deny contains msg if {
	doc := helmreleases[_]
	is_webhook_release(doc)
	group_name := object.get(values(doc), "groupName", "")
	trim_space(group_name) == ""
	name := object.get(metadata(doc), "name", "<unknown>")
	msg := sprintf("Namecheap webhook HelmRelease %s must set values.groupName", [name])
}

deny contains msg if {
	doc := helmreleases[_]
	is_webhook_release(doc)
	replicas := object.get(values(doc), "replicaCount", 0)
	replicas <= 0
	name := object.get(metadata(doc), "name", "<unknown>")
	msg := sprintf("Namecheap webhook HelmRelease %s must set values.replicaCount", [name])
}
