...
EXAMPLE_DIR="$REPO_ROOT/infra/modules/traefik/examples/render"
POLICY_DIR="$REPO_ROOT/infra/modules/traefik/policy/manifests"

if ! command -v tofu >/dev/null 2>&1; then
  echo "tofu must be installed to run traefik render policy checks" >&2
  exit 1
fi

if ! command -v conftest >/dev/null 2>&1; then
  echo "conftest must be installed to run traefik render policy checks" >&2
  exit 1
fi

if ! command -v python3 >/dev/null 2>&1; then
  echo "python3 must be installed to run traefik render policy checks" >&2
  exit 1
fi
