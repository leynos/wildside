...
function checkTheme(file, threshold) {
	...
	if (!brand || !accent) {
		errors.push(
			`Missing brand/accent in ${file instanceof URL ? file.pathname : file}`,
		);
		return errors;
	}
	...
	for (const [label, fgRef, bgRef] of pairs) {
		...
		if (fgRef == null || bgRef == null) {
			errors.push(
				`${label} in ${fileHint} is missing a value or contrast token`,
			);
			continue;
		}
		try {
...
console.log(
	`Contrast checks passed for themes (threshold: ${contrastThreshold}).`,
);
process.exit(0);
